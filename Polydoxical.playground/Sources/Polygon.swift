...
public class Polygon {
    ...
    /// Points that create the polygon
    public var points: [CGPoint]
    
    public var sides: Int
    
    init(sideLength: CGFloat, path: CGPath, points: [CGPoint], sides: Int) {
    ...
    public static func create(polygonOf sides: Int, at point: CGPoint, size: CGFloat, offset: CGFloat=0, shift: Int? = nil) -> Polygon? {
        let path = CGMutablePath()
        var points = createArrayOfPoints(sides: sides, at: point, radius: size, offset: offset)
        if let shift = shift, shift != 0 && shift <= points.count {
            points = points.shiftRight(amount: shift)
        }
        ...
        return Polygon(sideLength: sideLength, path: path, points: points, sides: sides)
    }
}
