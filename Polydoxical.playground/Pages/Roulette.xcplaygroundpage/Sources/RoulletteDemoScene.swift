...
public class RouletteScene: SKScene {
    ...
    public func rotateReferencePoint(percentage: Float) {
        ...
        for node in rotatingNodes {
            ...
            let point = 2 * Double.pi * Double(-percentage)
    ...
    /// Updates the length of the prolate or curtate example
    ///
    /// - Parameters:
    ///   - length: new length
    ///   - type: the type of roulette that needs to have it's length updated
